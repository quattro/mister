...
beta_p <- rep(rnorm(1, sign * 0.001, sqrt(h2g / Msnps)), Msnps)
...
res_list <- list(ivw_mr(scans) %>% mutate(Method = "IVW"),
                 ivw_mr(scans, mode = "scaled") %>% mutate(Method = "IVW-S"),
                 ivw_mr(scans, mode = "random") %>% mutate(Method = "IVW-RE"),
                 egger_mr(scans) %>% mutate(Method = "Egger"),
                 egger_mr(scans, mode = "scaled") %>% mutate(Method = "Egger-S"),
